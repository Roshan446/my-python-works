...
# Q print year dictonary

# film_dict ={}

# for film in movies:
#     if film.get("year") in film_dict:
#         film_dict[film.get("year")] += 1
#     else:
#         film_dict[film.get("year")] = 1
# print(film_dict)


#print the no of movies

print(len(movies))

# Q3 print all films that have run time greater than 150

# for film in movies:
#     if int(film.get("runtime"))>150:
#         print(film.get("title"))



run_time_movies = [film.get("title")for film in movies if int(film.get("runtime")) > 150]
print(run_time_movies)


#Q4 greatest run time 
name = ""
starting_run = 0
for film in movies:
    if int(film.get("runtime")) > starting_run:
        starting_run = int(film.get("runtime"))
        name = film.get("title")
print(name)


highest_runtime = max(movies, key = lambda run: int(run.get("runtime")))
...
#     if int(v)>initial_value:
#         initial_value = int(v)
#         initial_name = k
# print(initial_name)

value_key = [(v,k) for k, v in film_dict.items()]
print(max(value_key))

# Q print movie from id from the input
id_fetch = int(input("please enter the id? "))

for id in movies:
    if id_fetch == id.get("id"):
        print(id.get("title"))

comedy_drama =[cd.get("title") for cd in movies if "Comedy" in cd.get("genres") and "Drama" in cd.get("genres")]
print(comedy_drama)
...
for film in movies:
    for v in film.get("genres"):
        if v in genre_dict:
            genre_dict[v] +=1
        else:
            genre_dict[v] =  1

val_key = [(v,k) for k,v in genre_dict.items()]
print(max(val_key))